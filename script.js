      ...
      function populateDates() 
      {
        ...
        { //iterate through each day
          ...
          <tr>
            ...
            <td class="overtime-pay" id="overTimePay${rowId[i]}">
                  <input type="checkbox" id="checkBox${rowId[i]}">
                  <label for="checkBox"></label>
            </td>
            <td class="clear-row" id="clearRow${rowId[i]}">
                  <input type="button" id="clearRow${rowId[i]}" value="&#128465;">
            </td>
          ...
          $('.clear-row input[type="button"]').on('click', function() {
            // Reset the input values within the row
            let rowId = $(this).attr('id').replace('clearRow', ''); // Get the row ID
            resetInputValues(rowId);
          });
        
          // Function to reset the input values
          function resetInputValues(rowId) {
            // Perform the necessary steps to reset the input values for the specified row
            // For example, you can use the row ID to target the specific inputs and set their values to empty
            $(`#startTime${rowId}`).val('');
            $(`#finishTime${rowId}`).val('');
            $(`#break${rowId}`).val('0');
            $(`#hoursWorked${rowId}`).val('');
